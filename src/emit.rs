...
fn emit_actor(actor: &ActorDecl, model: &Model<'_>) -> Result<String> {
    ...
    // Lower entries first so state-layout emission can omit payload structs
    // that straight-line route-local materialization made unnecessary.
    let entry_sil = actor
        .entries
        .iter()
        .map(|entry| {
    ...
    emit_state_layouts(&mut out, actor, model, &entry_sil)?;
...
fn emit_state_layouts(out: &mut String, current_actor: &ActorDecl, model: &Model<'_>, entry_sil: &[String]) -> Result<()> {
    ...
    for state_name in state_names {
        ...
        let payload_type = hidden_state_payload_type_name(&state_name);
        if state_payload_type_name(&state_name, model) != state_name && entry_sil.iter().any(|entry| entry.contains(&payload_type)) {
            out.push_str(&format!("    struct {payload_type} {{\n"));
...
impl<'a, 'm> BodyLowerer<'a, 'm> {
    ...
    fn lower_plain_statement(&mut self, out: &mut String, indent: usize) -> Result<()> {
        ...
        if let Some((source_ty, name, expr)) = parse_typed_local_statement(&statement) {
            ...
            if let Some(target_actor) = self.materialized_route_target_for_local(source_ty, name, expr, &ty)? {
    ...
    fn materialized_route_target_for_local(
        &self,
        source_ty: &str,
        name: &str,
        expr: &str,
        ...
        // A payload-typed object used only as one concrete route's state can
        ...
        // read, rewrite, or second route preserves the neutral payload.
        if self.conditional_depth != 0
            || lowered_ty != hidden_state_payload_type_name(source_ty)
...
mod tests {
    ...
    fn foreign_routes_materialize_the_target_actors_cut() {
        ...
        assert!(source_sil.contains("gen__a_state next = {"), "{source_sil}");
...
"#,
    ...
    #[test]
    fn single_route_locals_materialize_early_while_branch_choices_keep_payloads() {
        let straight_path = PathBuf::from("examples/route_payloads.ag");
        let straight_source = fs::read_to_string(&straight_path).expect("route payload example exists");
        let straight_module = crate::parser::parse_module(straight_path.clone(), straight_source).expect("example parses");
        ...
        assert!(!straight_sil["Lobby"].contains("gen__board_state_payload"), "{}", straight_sil["Lobby"]);
        assert!(straight_sil["Lobby"].contains("BoardState next_board = {"), "{}", straight_sil["Lobby"]);
        assert!(!straight_sil["Mux"].contains("gen__archive_state_payload"), "{}", straight_sil["Mux"]);
        assert!(straight_sil["Mux"].contains("ArchiveState next_archive = {"), "{}", straight_sil["Mux"]);
        assert!(!straight_sil["Archive"].contains("gen__board_state_payload"), "{}", straight_sil["Archive"]);

        let choice_path = PathBuf::from("examples/route_payload_choice.ag");
        let choice_source = fs::read_to_string(&choice_path).expect("route payload choice example exists");
        ...
        assert!(choice_sil.contains("struct gen__board_state_payload {"), "{choice_sil}");
        assert!(choice_sil.contains("gen__board_state_payload next_board = {"), "{choice_sil}");
        assert!(
            choice_sil
