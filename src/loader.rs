...
pub fn load_program(root: impl AsRef<Path>) -> Result<Program> {
    ...
    let canonical_root =
        fs::canonicalize(&root).map_err(|err| ArgentError::at(&root, err.to_string()))?;
...
impl Loader {
    fn load_module(&mut self, path: &Path) -> Result<()> {
        ...
        let source = fs::read_to_string(&canonical)
            .map_err(|err| ArgentError::at(&canonical, err.to_string()))?;
        let module = parse_module(canonical.clone(), source)?;
        let base = canonical
            .parent()
            .ok_or_else(|| ArgentError::at(&canonical, "module path has no parent"))?
            .to_path_buf();
